...
(function($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		pp_settings = jQuery.extend({
			...
			custom_markup: '',
			social_tools: '' +
				'<div class="twitter"><a href="http://twitter.com/share?text={text}&url={url}" class="twitter-share-button" data-count="none"><i class="icon-twitter"></i><span class="text_social">Tweet</span></a></div>' +
				'<div class="facebook"><a href="https://www.facebook.com/sharer.php?picture={img}&u={url}&link={url}&description={text}" class="fb-share-button" data-count="none"><i class="icon-facebook"></i><span class="text_social">Facebook</span></a></div>' /* html or false to disable */
		}, pp_settings);
		...
		$.prettyPhoto.open = function(event) {
			...
			if(settings.social_tools){
				...
				if(typeof pp_descriptions[set_position] != 'undefined' && pp_descriptions[set_position] != ""){
					var item_title = unescape(pp_descriptions[set_position]);
				}

				var path = location.pathname.split('/');
				path.splice(-1,1);
				var url = location.origin+path.join('/');

				link_info = settings.social_tools.replace(/{url}/g, encodeURIComponent(location.href))
					.replace(/{text}/g, encodeURI(document.title + ' : ' + item_title))
					.replace(/{img}/g, url+'/'+pp_images[set_position]);

				$pp_pic_holder.find('.pp_social').html(link_info);

				//jQuery('meta[property="og:image"]').attr('content', url+'/'+pp_images[set_position]);
			}
...
(function($){$.getQuery=function(query){query=query.replace(/[\[]/,"\\\[").replace(/[\]]/,"\\\]");var expr="[\\?&]"+query+"=([^&#]*)";var regex=new RegExp(expr);var results=regex.exec(window.location.href);if(results!==null){return results[1];return decodeURIComponent(results[1].replace(/\+/g," "))}else{return!1}}})(jQuery);$(function(){$('.pill-switch').click(function(){var current_url=window.location.href;window.location="?theme=pill"});$('.tab-switch').click(function(){var current_url=window.location.href;window.location="?theme=tab"});$('.image-switch').click(function(){var current_url=window.location.href;window.location="?theme=image"});$('.link-switch').click(function(){window.location="?theme=link"});if($.getQuery('theme')==='pill'){$(function(){$.scrollUp({animation:'fade',activeOverlay:'#00FFFF'})});$('.pill-switch').addClass('active');$('#scrollUpTheme').attr('href','css/themes/pill.css?1.1')}else if($.getQuery('theme')==='link'){$(function(){$.scrollUp({animation:'fade',activeOverlay:'#00FFFF'})});$('#scrollUpTheme').attr('href','css/themes/link.css?1.1');$('.link-switch').addClass('active')}else if($.getQuery('theme')==='image'){$(function(){$.scrollUp({animation:'fade',activeOverlay:'#00FFFF',scrollImg:{active:!0,type:'background',src:'img/top.png'}})});$('#scrollUpTheme').attr('href','css/themes/image.css?1.1');$('.image-switch').addClass('active')}else{$(function(){$.scrollUp({animation:'slide',activeOverlay:'#00FFFF'})});$('#scrollUpTheme').attr('href','css/themes/tab.css?1.1');$('.tab-switch').addClass('active')}
$('#toggleActive').click(function(){$('#scrollUp-active').toggle();var text=$(this).text()=='Hide activeOverlay'?'Show activeOverlay':'Hide activeOverlay';$(this).text(text).toggleClass('active')})});
